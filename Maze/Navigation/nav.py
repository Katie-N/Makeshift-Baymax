...
class MazeNavigator(Node):
    def __init__(self):
        ...
        self.grid = self.load_grid('map.txt')
    ...
    def generate_path(self):
        """
        Generates a simple test path along the top row where cells are 0 (path).
        This will be replaced with shortest-path algorithm later.
        """
        path = []
        for col in range(len(self.grid[0])):
            if self.grid[0][col] == 0:
                path.append((0, col))  # Add cell at row 0, current column
        return path
