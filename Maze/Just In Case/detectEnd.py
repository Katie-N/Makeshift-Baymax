...
class ColorTracking(Node):
    def __init__(self, name):
        ...
        self.enemy_coords_publisher = self.create_publisher(Point, '/blueFinishLine', 10)
    ...
    def listener_callback(self, data):
        ...
        # Process image
        # This extracts the blue square
        dark_bound = np.array([110, 58, 10])  # Lower bound for the blue (BGR)
        light_bound = np.array([199, 106, 20])
        blue_mask = cv2.inRange(gaussian, dark_bound, light_bound)

        # Get the color's centroid x and y components ( we compute this in a custom function below )
        centroid_x, centroid_y = self.get_color_centroid( blue_mask )
