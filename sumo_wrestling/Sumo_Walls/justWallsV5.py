...
class WallDetector(Node):
    ...
    def lidar_callback(self, msg):
        ...
        wall_data = {
            "left": self.get_wall_info(distances, angles, 45, 135),  # Left side (90° to 110°)
            "right": self.get_wall_info(distances, angles, 225, 315),  # Right side (-90° to -110°)
            "front": self.get_wall_info(distances, angles, -45, 45),  # Front (-10° to 10°)
            "back": self.get_wall_info(distances, angles, 135, 225)  # Back (170° to 190°)
        }
    ...
    def get_wall_info(self, distances, angles, angle_min, angle_max):
        ...
        Returns:
        - (closest_distance, closest_angle): Tuple containing the nearest detected wall distance and the angle the closest point of the wall is at
        """
        ...
        if len(valid_distances) > 0:
            ...
            closest_point_index = np.argmin(valid_distances)
            closest_distance = valid_distances[closest_point_index]
            closest_points_angle = valid_angles[closest_point_index]
            return closest_distance, closest_points_angle
        else:
