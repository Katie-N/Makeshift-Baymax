...
class ColorTracking(Node):
    def __init__(self, name):
        ...
        self.enemy_coords_publisher = self.create_publisher(Float32MultiArray, '/ball_position', 10)
        self.greenOpp = True
    ...
    def listener_callback(self, data):
        ...
        if (self.greenOpp): # For a green opponent goal
            ...
            lower_bound = np.array([54, 78, 44])  # Dark Green
            upper_bound = np.array([146, 194, 131])  # Light green
        else: # The opponent's goal is purple
            lower_bound = np.array([66, 23, 79])  # Dark Purple
            upper_bound = np.array([227, 158, 223])  # Light Purple
        goal_mask = cv2.inRange(gaussian, lower_bound, upper_bound)
